    ...
    for j=1:size(Matrix{i},1)
        for k=1:size(Matrix{i},2)
            ...
            for n = 1:length(w2)
                ...
                if i == 8 % We are at L
                    L_Sensitivity{m, n} = (sum(abs(Gx_L(J, Xph, I, D, K, M, t1, t2, C, w2(n))), 2) - sum(abs(Gx_L(J, Xmh, I, D, K, M, t1, t2, C, w2(n))), 2))./h .* parameter./sum(abs(Gx_L(J_org, L, I, D, K, M, t1, t2, C, w2(n))), 2);
