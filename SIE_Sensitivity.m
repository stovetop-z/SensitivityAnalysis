        ...
        for k=1:size(Matrix{i},2)
            ...
            for n = 1:length(w2)
                if i == 1 % We are at I
                    Xph(col, row) = xph;
                    Xmh(col, row) = xmh;
                    I_Sensitivity{m, n} = (abs(G_Paul(Xph, D, K, M, t1, t2, C, w2(n))) - abs(G_Paul(Xmh, D, K, M, t1, t2, C, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));
                ...
                if i == 2 % We are at D
                    Xph(col, row) = xph;
                    Xmh(col, row) = xmh;
                    D_Sensitivity{m, n} = (abs(G_Paul(I, Xph, K, M, t1, t2, C, w2(n))) - abs(G_Paul(I, Xmh, K, M, t1, t2, C, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));                
                end

                if i == 3 % We are at K
                    Xph(col, row) = xph;
                    Xmh(col, row) = xmh;
                    K_Sensitivity{m, n} = (abs(G_Paul(I, D, Xph, M, t1, t2, C, w2(n))) - abs(G_Paul(I, D, Xmh, M, t1, t2, C, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));                
                end
    
                if i == 4 % We are at C
                    C_Sensitivity{m, n} = (abs(G_Paul(I, D, K, M, t1, t2, Xph, w2(n))) - abs(G_Paul(I, D, K, M, t1, t2, Xmh, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));                
                end
    
                if i == 5 % We are at M
                    M_Sensitivity{m, n} = (abs(G_Paul(I, D, K, Xph, t1, t2, C, w2(n))) - abs(G_Paul(I, D, K, Xmh, t1, t2, C, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));                
                end
    
                if i == 6 && t1_count == 0 % We are at t1
                    t1_Sensitivity{m, n} = (abs(G_Paul(I, D, K, M, t1 + t1*sqrt(eps), t2, C, w2(n))) - abs(G_Paul(I, D, K, M, t1 - t1*sqrt(eps), t2, C, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));                
                ...
                if i == 7 && t2_count == 0 % We are at t2
                    t2_Sensitivity{m, n} = (abs(G_Paul(I, D, K, M, t1, t2 + t2*sqrt(eps), C, w2(n))) - abs(G_Paul(I, D, K, M, t1, t2 - t2*sqrt(eps), C, w2(n))))./h .* parameter./abs(G_Paul(I, D, K, M, t1, t2, C, w2(n)));                
