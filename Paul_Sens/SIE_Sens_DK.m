    ...
    for j=1:size(Matrix{i},1)
        for k=1:size(Matrix{i},2)
            ...
            if(i == 2 || i == 3) && (row == 3 && col == 2)
                ...
                Xph(row, col) = xph;
                Xmh(row, col) = xmh;
            elseif matrix(row, col) == 0 || ((i == 1 || i == 2 || i == 3) && row > col)
                continue
            else
                 % The parameter of interest is going to be set to the current
                % row and column of the matrix
            ...
            for n = 1:length(w2)
                if i == 1 % We are at I
                    I_Sensitivity{m, n} = (abs(Orig_G(Xph, D, K, M, t1, t2, C, w2(n))) - abs(Orig_G(Xmh, D, K, M, t1, t2, C, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));
                end
    
                if i == 2 % We are at D
                    D_Sensitivity{m, n} = (abs(Orig_G(I, Xph, K, M, t1, t2, C, w2(n))) - abs(Orig_G(I, Xmh, K, M, t1, t2, C, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));                
                end

                if i == 3 % We are at K
                    K_Sensitivity{m, n} = (abs(Orig_G(I, D, Xph, M, t1, t2, C, w2(n))) - abs(Orig_G(I, D, Xmh, M, t1, t2, C, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));                
                end
    
                if i == 4 % We are at C
                    C_Sensitivity{m, n} = (abs(Orig_G(I, D, K, M, t1, t2, Xph, w2(n))) - abs(Orig_G(I, D, K, M, t1, t2, Xmh, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));                
                end
    
                if i == 5 % We are at M
                    M_Sensitivity{m, n} = (abs(Orig_G(I, D, K, Xph, t1, t2, C, w2(n))) - abs(Orig_G(I, D, K, Xmh, t1, t2, C, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));                
                end
    
                if i == 6 && t1_count == 0 % We are at t1
                    t1_Sensitivity{m, n} = (abs(Orig_G(I, D, K, M, t1 + t1*sqrt(eps), t2, C, w2(n))) - abs(Orig_G(I, D, K, M, t1 - t1*sqrt(eps), t2, C, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));                
                ...
                if i == 7 && t2_count == 0 % We are at t2
                    t2_Sensitivity{m, n} = (abs(Orig_G(I, D, K, M, t1, t2 + t2*sqrt(eps), C, w2(n))) - abs(Orig_G(I, D, K, M, t1, t2 - t2*sqrt(eps), C, w2(n))))./h .* parameter./abs(Orig_G(I, D, K, M, t1, t2, C, w2(n)));                
